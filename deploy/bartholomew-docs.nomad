...
variable "oci_ref" {
  type        = string
  description = "The OCI reference of the Spin app for the Bartholomew Docs website"
}
  ...
  group "bartholomew-docs" {
    ...
    task "server" {
      ...
      template {
        ...
        IFS=/ read -r registry image <<< "${var.oci_ref}"
        aws ecr get-login-password --region ${var.region} | \
          spin registry login --username AWS --password-stdin $registry
